...
from src.transformer import Linear2, TokenEmbedding, RMSNorm, SiLU, FFN, RotaryPositionalEmbedding, Utils
...
def run_linear(
    ...
    #     "weights": weights
    # }
    linear = Linear2(in_features=d_in, out_features=d_out)
...
def run_embedding(
    ...
    embeddingMap = TokenEmbedding(num_embeddings=vocab_size, embeddings_dim=d_model)
    embeddingMap.load_state_dict({
        "mapping": weights
    })
    return embeddingMap(token_ids)
...
def run_softmax(in_features: Float[Tensor, " ..."], dim: int) -> Float[Tensor, " ..."]:
    ...
    """
    return Utils.stable_softmax(x=in_features, dimension=dim)
